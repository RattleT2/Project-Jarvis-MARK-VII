...
def _load_api_key() -> str:
    """
    ✅ REFACTORED: Get OpenRouter API key from env or config.
    Uses centralized API key management when available.
    """
    try:
        from utils.api_utils import get_openrouter_api_key
        ...
        try:
            ...
            raise RuntimeError(
                ...
                f"or create api_keys.json at: {API_KEY_PATH}"
            )
...
REQUEST_TIMEOUT       = 15   # ✅ FIX: Reduced from 45s to prevent hanging
MAX_RETRIES_PER_MODEL = 2    # attempts before moving to next model
RETRY_DELAY           = 1    # seconds between retries (optimized from 2)
    ...
    def _call(
        ...
        for attempt in range(1, MAX_RETRIES_PER_MODEL + 1):
            try:
                ...
                # ✅ FIX: Handle authentication errors (401, 403)
                if resp.status_code == 401:
                    logger.error(
                        f"[OpenRouter] {model} → HTTP 401 UNAUTHORIZED. "
                        "Your API key is invalid. Check OPENROUTER_API_KEY environment variable "
                        "or openrouter_api_key in config/api_keys.json"
                    )
                    # Don't retry - authentication error is not transient
                    return None
                
                if resp.status_code == 403:
                    logger.error(
                        f"[OpenRouter] {model} → HTTP 403 FORBIDDEN. "
                        "Access denied. Check your API key permissions or account status."
                    )
                    return None
                ...
                # ✅ FIX: Better error logging for other HTTP errors
                logger.warning(
                    f"[OpenRouter] {model} → HTTP {resp.status_code} "
                    f"(attempt {attempt}/{MAX_RETRIES_PER_MODEL}) - {resp.text[:100]}"
                )
