...
class _LiveSession:
    ...
    def start(self, player=None, suppress_output: bool = False):
    ...
    async def _recv_loop(self):
        ...
        try:
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript_buf and self._player and not self._suppress_output:
...
def _ensure_started(player=None, suppress_output: bool = False):
    global _started
    with _start_lock:
        if not _started:
            _live.start(player=player, suppress_output=suppress_output)
            _started = True
        elif player is not None:
            _live._player = player
            _live._suppress_output = suppress_output
...
def screen_process(
    ...
    _ensure_started(player=player, suppress_output=suppress_output)
...
def warmup_session(player=None, suppress_output: bool = False):
    try:
        _ensure_started(player=player, suppress_output=suppress_output)
    except Exception as e:
