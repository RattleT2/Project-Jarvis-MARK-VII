...
def _should_extract_via_gemini(user_text: str, jarvis_text: str, api_key: str) -> bool:
    try:
        ...
        logger.warning(f"[Memory] Gemini stage1 fallback failed: {e}")
        return False
...
def should_extract_memory(user_text: str, jarvis_text: str, api_key: str = "") -> bool:
    ...
    try:
        ...
        print(f"[Memory] ⚠️ Stage1 check failed (OpenRouter): {e}")

    if openrouter_failed and api_key:
        print("[Memory] 🔄 Falling back to Gemini for memory check...")
        return _should_extract_via_gemini(user_text, jarvis_text, api_key)

    return False


def _extract_via_gemini(user_text: str, jarvis_text: str, api_key: str) -> dict:
    try:
        import google.generativeai as genai
        genai.configure(api_key=api_key)
        ...
        response = model.generate_content(
            ...
            f"  projects      → projects being built, ongoing work, goals\n"
            ...
            f' "notes":{{"works_at_night":{{"value":"usually active at late night"}}}}}}\n\n'
            f"Conversation:\n{combined}\n\nJSON:",
        ...
        logger.warning(f"[Memory] Gemini extraction fallback failed: {e}")
        return {}
...
def extract_memory(user_text: str, jarvis_text: str, api_key: str = "") -> dict:
    ...
    try:
        ...
        openrouter_failed = True
        if "429" not in str(e):
            print(f"[Memory] ⚠️ Extract failed (OpenRouter): {e}")

    if openrouter_failed and api_key:
        print("[Memory] 🔄 Falling back to Gemini for memory extraction...")
        return _extract_via_gemini(user_text, jarvis_text, api_key)
